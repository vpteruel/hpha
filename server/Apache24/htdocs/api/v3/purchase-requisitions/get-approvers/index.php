    ...
    if (isset($_GET['email']) && isset($_GET['function-centre-number'])) {
        ...
        if ($email === 'vinicius.teruel@[domain]') {
            ...
            if ($department_roles !== null) {
                ...
                $result->is_current_user_manager = is_manager($email, $department_roles); 
...
function is_manager($email, $department_roles) {
    return ($email === $department_roles['manager_email']) ? 'yes' : 'no';
}
?>
